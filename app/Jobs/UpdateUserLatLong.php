...
use App\Jobs\ProcessUserLatLongChunk;

class UpdateUserLatLong implements ShouldQueue
{
    ...
    public function __construct($tempDBname,$created_at)
    {
    ...
    public function handle()
    {
       ...
       // $GoggleAPIKey = env('GOOGLE_API_KEY');
        ...
        try {
            ...
            $SubUserAddressesChunk = SubUserAddresse::whereNotNull('address')
                ->where('address', '!=', '')
                ...
                //->chunk(100, function ($SubUserAddressesChunk)  use ($GoggleAPIKey) {
                    foreach ($SubUserAddressesChunk as $SubUserAddresses) {
                        try {

                            ProcessUserLatLongChunk::dispatch($SubUserAddresses->toArray(), $tempDBname)->onQueue('default');
                            ...
                            //     $officeDistance  = $this->calculateDistance($Complat,$Complong,$latLong['lat'],$latLong['lng'],$GoggleAPIKey);
                            // }else {
                            //     $officeDistance = null;
                            // }
                            // DB::update(
    ...
    // public function calculateDistance($officeLat, $officeLng, $employeeLat, $employeeLng,$GoggleAPIKey){

    //     $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins={$officeLat},{$officeLng}&destinations={$employeeLat},{$employeeLng}&key={$GoggleAPIKey}";
